...
\item The prominent $^{12}$C correlation band is then fitted with a linear function to extract its slope (Figure~Z).
\item Based on the fitted slope, the histogram is rotated such that the $^{12}$C band is vertically aligned. A 1D projection onto the abscissa is performed (Figure~ZZ).
