...
\begin{itemize}
\itemsep0em
\item MWPC2 and MWPC3 hit-level data:\newline
...
\]
Figure \ref{fig:hyp_function} shows the "hyperbolic squared secant" function with the insertef values for $Q_{max}$, $Q_R$ and $Q_L$. 
\begin{figure}[htpb]
    \centering
    \includegraphics[width=\textwidth,height=8cm,keepaspectratio=true]{Figures/hyperbolic_squared_secant_function.png}
    \caption{
   	 Firgure taken from \cite{martin2021fission}, with w being the with of the cathode pads of the MWPC and $a_2$ the final position value of the hit determined by the hyperbolic squared secant function (in red). In black the measured charge deposition distribution in the MWPC. 
     }
    \label{fig:hyp_function}
\end{figure}


\item MWPC2 and MWPC3 data with own "hit-clustering" level:
\item MWPC1 and MWPC3 hit-level data:
\item MWPC1 and MWPC3 data with own "hit-clustering" level:
\item MWPC2 and MWPC3 hit-level data, right side:
\item MWPC2 and MWPC3 data with own "hit-clustering" level, right side:

\end{itemize}



%-> explain that ratio is done
%-> N2_12C = R12_c/carbon*N_x carbon
%-> no det. efficiencies on MW2 and MW3 have to be applied, since ratio used and det. efficiency of MWPCs should be same for all carbon isotopes as only charge important
%-> however positional efficiency not corrected: TODO-> maybe to be discussed
%-> discuss uncorrelated stuff on y axis
%-> present algorithm for diagonal cut:
%		-> using own clustering algorithm
%		-> no clustering algorithm
%		-> using mw13 with own clustering algorith
%		-> using mw13 with no clustering algorighm
%		-> using mw23 only upper side, own clustering algorithm
%		-> using mw23 onyl upper side, no clustering algorithm
