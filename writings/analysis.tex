...
\begin{enumerate}
    ...
    The flipped and measured distributions are compared on a bin-by-bin basis. As illustrated in Fig.~\ref{fig:geo_correction}, lower panel, the ratio between the two distributions drops significantly around $x \approx -20\,\text{mm}$, indicating the onset of acceptance loss. This reduced level persists up to approximately $x \approx -25\,\text{mm}$.

    \item \textbf{Calculation of the correction factor:}  
    To estimate the number of missing hits due to limited acceptance, the bin contents of both the flipped and measured distributions are summed over the range from the edge of the active area of MWPC1 ($x = -110\,\text{mm}$) up to the approximate limit of reliable detection ($x = -25\,\text{mm}$ or $x = -20\,\text{mm}$). The correction factor is then computed as:
    \begin{equation}
        \text{geo\_corr}_{x} = 1 + \frac{\sum_{x = -110}^{-25/-20} (h_{\text{flipped}} - h_{\text{full})}}{\sum_{x = -110}^{ +110} h{\text{full}}}
    \end{equation}
    The final value of $\text{geo\_corr}_x$ is taken as the average of the results obtained using $x = -25\,\text{mm}$ and $x = -20\,\text{mm}$ as cutoffs. The uncertainty is estimated from the spread between the minimum, maximum, and mean values in this interval.
...
\begin{equation}
    \text{geo}_{xy} = \text{geo\_corr}_y \cdot \text{geo\_corr}_y
\end{equation}
...
\begin{equation}
\sigma_{\text{corr}} = -\frac{1}{N_t} \ln(\text{geo}{xy}^{\text{target}}) + \frac{1}{N_t} \ln(\text{geo}{xy}^{\text{empty}})
...
An overview of the resulting correction factors geoxygeoxy​ and the corresponding correction terms σcorrσcorr​ is presented in Fig.~\ref{fig:geo_corrections_overview}, stratified by target material and beam energy. The error bars reflect the uncertainty in the estimated onset of acceptance loss in both the xx- and yy-distributions. Specifically, they are derived from the variation of the correction factor when shifting the integration bounds between, e.g., x=−30 mmx=−30mm and x=−20 mmx=−20mm (and analogously in yy).

\textit{[TODO: Insert formal expression for error propagation of the correction factor here.]}

The correction term σcorrσcorr​ is then subtracted from the preliminary estimate of the charge-changing cross section to yield the final result:

\begin{equation}
\sigma_{\text{cc}}^{\text{final}} = \sigma_{\text{cc}}^{\text{prelim}} - \sigma_{\text{corr}}
\end{equation}

Figure~\ref{fig:xsec_prelim} shows the preliminary estimates before applying geometric acceptance corrections, while the fully corrected charge-changing cross sections are presented in Fig.~\ref{fig:xsec_corrected}.\\
