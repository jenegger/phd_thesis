...
\begin{enumerate}
\itemsep0em
\item Correction for the x-position distribution:
%TODO: insert x-Distribution in MW1 and the fitted functions
\begin{enumerate}
...
\end{equation}
\end{enumerate}
\item Correction for the y-position distribution:
%TODO: insert y-Distribution in MW1 and the fitted functions
\begin{enumerate}
...
\end{equation}
%\item First fit x-distribution with double-gaussian function ($A\,e^{\frac{(x-\mu)^2}{\sigma_A^{2}}} + B\,e^{\frac{(x-\mu)^2}{\sigma_B^{2}}}$) and get mean($\mu$) from fit.
%\item Fit again within range $\mu\,\pm21 [mm]$ with the double gaussian fit function. The obtained parameters are used for the following integration steps.
%\item Fit function is integrated in the range from -100 up to $\mu - 21 mm$ ($I_{fit\textunderscore left}$) and compared with the number of data entries in this region ($I_{data\textunderscore left}$).
%\item Fit function is integrated in the range from $\mu + 21$ up to 100 mm ($I_{fit\textunderscore right}$) and compared with the number of data entries in this region ($I_{data\textunderscore right}$).
%\item To reconstruct the number of carbon isotopes lost due to the reduced geometric efficiency the diffence between the integrated fit function and the data entries are compared. For the right integration region the difference between integrated number of entries and the fit integral ($\Delta_{right}$) should be small since this side should not be affected by the limited geometric efficiency and the data should be well described by the double gaussian fit. For the left integration region this difference ($\Delta_{left}$) is large since this side is prevalently affected by the geometric efficiency decrease, see figure \ref{fig:mw1_xy}. Since the data distribution should be symmetric with respect to the two integration regions the number lost data entries due to geometric efficiency limitations can be reconstructed by:\newline
%$\Delta_x = \Delta_{right} - \Delta_{left}$
\end{enumerate}
%\item Correction for the y-position distribution:
%\begin{enumerate}
%\item Fit y-distribution with double-gaussian function (free parameter, mean value of both gaussian fucntions equal) and get mean from fit.
%\item Fit function is integrated in the range of -100 up to mean - fix value and compared with the number of data entries in this range
%\item Fit function is integrated in the range of mean + fix value up to 100 and compared with the number of data entries in this range
%\item (data_integral_left_y -integral_fit_left_y) - (data_integral_right_y -integral_fit_right_y) explain compensation
%\end{enumerate}
%\end{enumerate}
\begin{figure}[htpb]
    \centering
...
\begin{figure}[htpb]
    ...
    \caption{
    Charge changing cross section with applied geometry corrections. In gray: charge changing cross section measurements without ... 
     }
    \label{fig:geo_corr_twim}
\end{figure}
Figure blabla shows the measured charge changing cross section once without considering the the limited geometric acceptance of the TWIM Music and once applying the geometric correction factors as presented in equation \ref{eq:geo_corr_cccs}. While the measurements for the
