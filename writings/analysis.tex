...
\end{figure}
\item MWPC2 and MWPC3 data with own "hit-clustering" level: To overcome the issue with potentially wrong x-position reconstruction in the MWPCs the event selection on MWPC2 and MWPC3 was restricted to events where both MWPC2 and MWPC3 have only one spatially constrained cluster(see figure \ref{fig:own_clustering}) to avoid ambiguities in the position determination.
%\begin{figure}[htpb]
%    \centering
%    \includegraphics[width=\textwidth,height=8cm,keepaspectratio=true]{Figures/own_clustering_mwpcs.png}
%    \caption{
%   	 
%     }
%    \label{fig:own_clustering}
%\end{figure}
\begin{figure}
