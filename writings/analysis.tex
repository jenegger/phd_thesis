...
\Delta\sigma_\text{stat.} = \sqrt{\left(\frac{\partial \sigma_{cc}}{\partial N_t} \Delta N_t\right)^2 + \left(\frac{\partial \sigma_{cc}}{\partial P_{surv}(Z=6)} \Delta P_{surv}(Z=6)\right)^2 + \left(\frac{\partial \sigma_{cc}}{\partial P^{E}_{surv}(Z=6)} \Delta P^{E}_{surv}(Z=6)\right)^2}
\end {equation}
where $\Delta N_t$ accounts for the uncertainty in the measurement of the target thickness. $P_{surv}$ follows a binomial distribution with $N_1$ independent experiments and a boolean valued reaction output (suvived vs. non-survived). Therefore $P_{surv}$ follows the standard error of binomial distributed variable:
\begin{equation}
\Delta P^{(E)}_{surv}(Z=6) = \sqrt{\frac{P^{(E)}_{surv}(Z=6)\cdot (1-P^{(E)}_{surv}(Z=6))}{N^{(E)}_{1}}}
\end{equation}
Inserting in equation \ref{eq:stat_cc} gives:
\begin{equation}
\Delta\sigma_\text{stat.} = \sqrt{\left(\frac{\sigma_I \Delta N_t}{N_t}\right)^2 + \frac{1}{N_t^2} \left(\frac{1 - P_{surv}(Z=6)}{P_{surv}(Z=6) N_1} + \frac{1 - P^{(E)}_{surv}(Z=6)}{P^{(E)}_{surv}(Z=6) N_1^E}\right)}
\end{equation}
...
\begin{equation}
\Delta\sigma_\text{stat.|iso} = \sqrt{\left(\frac{\sigma_I \Delta N_t}{N_t}\right)^2 + \frac{1}{N_t^2} \left(\frac{1 - P_{surv|iso}}{P_{surv|iso} N_1} + \frac{1 - P^{(E)}_{surv|iso}}{P^{(E)}_{surv|iso} N_1^E}\right)}
\end{equation}
