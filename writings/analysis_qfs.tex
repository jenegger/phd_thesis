\section{Qualitative Analysis -- Quasi-Free Scattering $^{12}$C(p,2p)$^{11}$B}\label{sec_analysis_qfs}
...
%Even though great improvements in the detector development  were achieved the correction factors to correct for geometric acceptance would be much too high ( $\approx \mathcal{O}(10)$) for precise cross section measurements for QFS-reactions since the correction factors would in turn rely on a a simplified reaction model. A precise analysis of the acceptance correction factor and the development of a more sophisticated and data driven reaction model is out of the scope of this work.\newline
%
...
While significant progress had been made in the development of the CALIFA detector system, limitations in the geometric acceptance remained. Specifically, the correction factors associated with detector coverage and geometry are of the order of $\mathcal{O}(10)$, depending on the beam energy. A high-precision geometric correction would require an extensive simulation of the CALIFA detector response and a dedicated analysis effort, which falls beyond the scope of the present work.\\




This analysis is dedicated to the identification of quasi-free scattering (QFS) reaction events. A prerequisite for this task is a precisely tuned reconstruction of the flight path of the reaction fragment downstream of the target, enabling unambiguous tagging of the fragment of interest, specifically $^{11}B$. This tagging is essential for the subsequent selection of events characterized by the detection of two high-energy protons in CALIFA.\\
Subsection~\ref{sec:fission_qfs} will introduce an additional, particularly ambitious application of QFS reactions: the successful pilot experiment S455, which investigated the fission process via the quasi-free scattering reaction $^{238}U(p,2pf)$. This study, along with its complementary Coulomb excitation fission (Coulex Fission) experiment, culminated in a publication in \textit{Nature}~\cite{morfouace2025asymmetric}.
...
As outlined in the introduction, the qualitative analysis presented in this work relies critically on the accurate identification of the $^{11}$B reaction fragment. This requires both precise tracking of the fragment's flight path downstream of the target and high-resolution time-of-flight (ToF) measurements, enabled by dedicated detector systems.\\
A fundamental prerequisite is the proper calibration of the relevant detectors, which is essential to ensure accurate, reliable, and physically meaningful measurements. For all detectors involved in the experiment -- except for the SOFIA (Study On FIssion with Aladin) Time-of-Flight Wall -- the calibration parameters provided by the respective detector expert groups were adopted.
This subsection therefore focuses on the calibration procedure of the SOFIA ToF Wall and describes in detail the reconstruction algorithm for the reaction fragment's flight path in the first section.
...
Since only one tracking detector downstream to GLAD, the MWPC3, was installed for the S444 experiment no angle of the deflected fragment/beam $\theta_{out}$ could be directly extracted. Herefore an advanced tracking algorithm was developed, motivated from Ref.~\cite{bertini2013study} (Sec. 3.4). 
\begin{itemize}
...
\item As in Fig.~\ref{fig:sub2_reco_path} shown, fit the data-points from previous step with a linear fit function and find the corresponding $\theta_{out}$ value where the fit line intersects with the abscissa. This corresponds to the case where $d1 = d2$. This is the approximated "kickpoint" in GLAD. 
...
The angle $\delta$ in Eq.~\ref{eq:rho_glad} is given by the trajectory line going through $(z_1,x_1)$/$(z_2,x_2)$ and the line parallel to the GLAD magnet width $L_{eff}$.\newline
...
\item \textbf{Arc trajectory within GLAD, ${l_{GLAD}}$:} This flightpath passage is determined by reconstructing $(z_1,x_1)$ and $(z_2,x_2)$, as it has been described in detail in the previous section. Hence the magnetic bending radius can be determined, see Eq.~\ref{eq:rho_glad}, and finally the arc trajectory ${l_{GLAD}}$ as in Eq.~\ref{eq:arc}.
...
A similar method was applied to select the specific boron isotope \(^{11}\mathrm{B}\). 
The lower boundary of the selection range was defined by the intersection point of the Gaussian-fitted distributions corresponding to the \(^{10}\mathrm{B}\) and \(^{11}\mathrm{B}\) peaks. 
The upper boundary was determined by measuring the distance between this intersection point and the \(^{11}\mathrm{B}\) peak maximum, and extending the same distance to the right side of the \(^{11}\mathrm{B}\) distribution.
...
The resulting correlations in azimuthal and polar angles, with the only cut to have at least two high energy clusters with energy larger 15 MeV, with respect to the simulated \textit{p2p}-simulations are shown in Fig.~\ref{fig:azimuth_corr} and Fig.~\ref{fig:polar_corr} accordingly.\newline
\begin{figure}[htpb]
...
\end{figure}
From the azimuthal correlation plot in Fig.~\ref{fig:azimuth_corr}, the geometric acceptance limitations of CALIFA become evident. In the so-called \textit{iPhos} region, CALIFA was instrumented only within the azimuthal ranges of $\pm 45^{\circ}$ and $157.5^{\circ}$ to $202.5^{\circ}$. Due to relativistic kinematics, both protons undergo a forward boost, leading to their predominant detection in the iPhos array. Consequently, any acceptance loss in this region has a substantial impact on the overall detection efficiency.\newline
