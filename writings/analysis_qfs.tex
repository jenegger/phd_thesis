...
Due to a lack of operational tracking detectors the tracking of the incoming beam onto the target did not allow for a sufficient precise reconstruction of the vertex in the 3 $\times$ 3 cm target. Also only one tracking detector downstream to GLAD, the MWPC3, was installed for the S444 experiment no angle of the deflected fragment/beam $\theta_{out}$ could be directly extracted. Herefore an advanced tracking algorithm was developed, motivated from Ref.~\cite{bertini2013study} (Sec. 3.4), the following steps describe the logics chain: 
\begin{itemize}
