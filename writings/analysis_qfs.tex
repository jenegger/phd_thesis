...
\begin{align*}
\widetilde{ToF} = \frac{$\overline{L[i]}$}{\frac{L[i]}{ToF[i]}}\footnote{$\overline{L[i]}$ is the mean pathlength, whereas $L[i]$ and $ToF[i]$ are eventwise selected}.
\end{align*}
The average time resolution $\sigma_t$ reconstructing the fligh-path as described in subsection \ref{subsec:flightpath_reco} and the mentioned time calibration steps results in $\approx 90 ps$\footnote{To reduce
