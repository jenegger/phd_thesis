...
%\end{enumerate}
\subsection{The GSI facility and FAIR}
The GSI Helmholtzzenrum f\"ur Schwerionenforschung GmbH was founded in 1969 (as "Gesellschaft f\"ur Schwerionenforschung mbH") looks back on a successful research history.Fundamental discoveries of superheavy elements, nearly 300 new isotopes including the double magic $^{100}$Sn isotope~\cite{hessberger2007gsi}, and groundbreaking studies of fundamental properties of nuclear matter go along with applications in medical physics, material science and atomic physics. All these experimental investigations are based on an extremely reliable and unique accelerator facility.\newline
The starting point for the production of relativistic heavy ions at GSI is a set of different ion sources. Depending on the experimental needs the ion sources at GSI are able to produce ions of all stable elements (from protons up to Uranium)\cite{ion_sources_web}. The only limitations arise from safety regulations, which currently prohibit the use of toxic primary beams, such as thallium (Tl).\newline
%cite here: https://www.gsi.de/en/researchaccelerators/accelerator_facility/ion_sources
On the first acceleration stage ions are injected from the ion source into the UNIversal Linear Accelerator (UNILAC). On a length of 120 meters ions are accelerated up to maximum energy of 11.4 AMeV. The low energy beam can be either directly used or is injected into the ring accelerator SIS18 (Schwerionensynchotron 18). Here the ion beam is further accelerated up to 4.7 GeV/u for protons or up to 1 GeV/u for Uranium. The magnets and  the ultra-high vacuum ($\sim$ $10^{-9}$ Pa) keep the ions well on their circular path (SIS18 has a circumference of 216 meters)\cite{sis18_web}. For the production of rare heavy isotopes the primary ion beam from SIS18 can be impinged on a light nuclear target, e.g beryllium, the so called production target. The secondary beams of radioactive isotopes are typically purified in the FRagment Separator (FRS). The FRS as a high-resolution magnetic spectrometer is capable to precisely select specific isotopes and to forward the desired beam of exotic relativistic nuclei to the various experiments or direct it to the Experimental Storage Ring (ESR).\newline
...
The First Science Phase, marking full SIS100 operation and complete FAIR commissioning, will be in 2028. Prior to the commissioning of SIS100, several high-priority experiments with significant scientific impact will take place as part of the Early Science Program in the newly established experimental halls. These include experiments utilizing the R3B setup in the High-Energy Cave (HEC), focusing on key aspects of nuclear structure and reactions under extreme conditions.\newline
An overview of the GSI and FAIR accelerator complex is shown in Fig.~\ref{fig:overview_gsi_fair}.
\begin{figure}[htpb]
    \centering
    \includegraphics[width=\textwidth,keepaspectratio=true]{Figures/final_fair_gsi_overview1.png}
    \caption{
	Schematic overview of the GSI and FAIR accelerator complex. The existing GSI facilities are shown in blue, including UNILAC and the SIS18. For the Early Science campaigns, the beams will be delivered from SIS18 to the high-energy branch of the NUSTAR experimental halls through the SuperFRS (e.g. to the HEC with R3B Setup). The First Science phase, indicated in green, includes the operation of the SIS100 synchrotron. Additional future extensions are marked in yellow (Rest-IO) and red (Rest-MSV). Figure adapted from Ref.~\cite{fair_reinmann}.
    }
    \label{fig:overview_gsi_fair}
\end{figure}


\subsection{The R3B Experiment}
The R3B (Reactions with Relativistic Radioactive Beams) experiment currently still operated  in Cave C at the GSI is a cutting-edge research experiment focused on the study of nuclear reactions and structure using high-energy radioactive ion beams. The experiment aims to investigate exotic nuclei far from stability, offering insights into the fundamental properties of nuclear matter, nucleosynthesis processes, and the forces governing nuclear interactions. A schematic overview of the R3B setup can be seen in Fig.~\ref{fig:r3b_setup_intro}.\newline
\begin{figure}[htpb]
    \centering
    %\includegraphics[width=\textwidth,height=6cm,keepaspectratio=true]{Figures/r3b_setup_intro.png}
    \includegraphics[width=\textwidth,keepaspectratio=true]{Figures/r3b_setup_sketch.png}
    \caption{
    Overview of the R3B setup in Cave C with detectors for the specific fragment branch identification.
    } 
    \label{fig:r3b_setup_intro}
\end{figure}
Currently short-lived isotopes are injected to the Cave C from the FRS, which preselects the isotopes of interest. The R3B setup is a fixed target experiment designed for kinematically complete reaction studies. To fulfill this requirement the incoming ions are tracked and identified on an event-by-event basis by dedicated detectors via time-of-flight and $\Delta$E measurement techniques \cite{nociforo2014time}. Depending on the settings and composition of the incoming ion beam different type of reactions take place in the target. All reaction products like heavy ions, neutrons, light charged particles and gamma rays are detected with large acceptance and spectroscopic resolution. For the detection of gammas and light charged ions from reactions with the target the dedicated CALIFA calorimeter (see more in Section~\ref{sec:califa}) and various tracking detectors are installed in the target region. The GLAD (GSI Large Acceptance Dipole) magnet, located at the center of the Cave C, allows fragment identification for the forward boosted heavy residues but also for beam like light fragments at the same time. The magnetic rigidity of the charged reaction residues is measured by a combination tracking detectors and a time of flight wall after the GLAD magnet. For the detection of the neutrons, not deflected by the magnetic field of GLAD, the new Large-Area Neutron Detector (NeuLAND)~\cite{boretzky2021neuland} is positioned after GLAD on the zero degree line with the incoming ion beam.\newline
The R3B setup offers high flexibility, allowing operation both in vacuum and in atmospheric conditions. Over the past few years, during the so-called \textit{Phase-0}, numerous experiments have been conducted, each utilizing different configurations and newly developed or upgraded detector systems.\newline  
The combination of a broad spectrum of incoming ion beams across a wide energy range, provided by the FRS facility, and the versatility of the R3B setup -- equipped with state-of-the-art detectors tailored for specific physics studies -- makes it an exceptional platform for experimental nuclear physics.

\subsection{R3B Setup in S444 Commissioning Experiment 2020}
The S444 Experiment of the FAIR Phase-0 program (see also Ref.~\cite{ponnath2024measurement}) for the commissioning of the CALIFA calorimeter in its final mechanical design took place in February 2020. The primary objective was to commission a new set of detectors together with the GLAD magnet using a stable $^{12}\mathrm{C}$ primary beam at four energy settings (400, 550, 650, and 800~AMeV). This commissioning served as a preparatory stage for the subsequent S467 experiment, which employs neutron-rich calcium isotopes as a medium-heavy incoming beam. The detectors for ion tracking, charge identification and time of flight measurement were adopted  from the SOFIA(Study on Fission with ALADiN\footnote{ALADiN magnet was the precessor of GLAD.}) collaboration.\newline
...
The tracking of the incoming ions as well as the heavy reaction products were performed by using Multi Wire Proportional Chambers (MWPCs). Similar multi wire proportional chambers were developed in late 1960s by George Charpak\footnote{George Charpak received the Nobel Prize in Physics in 1992 for his invention and development of particle detectors, in particular, the multi-wire proportional chambers.} at CERN\cite{charpak1968use}.\newline
The MWPC operates in the same way as aligned proportional counters with the difference of not having additional walls between the anode wires.\newline
In the general design the  MWCP is made up of a plane of anode wires enclosed between two cathode planes aligned parallel to the anode wire plane. Depending on the beam conditions the anode wires are set to high voltage ($\sim$ 1100 V) while the cathode planes refer to the ground.\newline
The volume between the two cathode planes is filled by a gas mixture of 84\% Argon and 16\% CO\textsubscript{2}. The decision of the gas mixture is driven by a balanced ratio between rate capability, amplification and quenching proprieties of the gas.\newline
When a charged particle passes through the detector it ionizes the gas. Primary electrons are created followed by a secondary ionization. The electron cloud drifts towards the wires (anodes) while the positive ions drift towards the grounded cathode planes. Close to the anode wire the electric field is high enough that the primary electrons can create an avalanche of secondary electrons to amplify the signal. As the MWPCs are operated in the proportional region, the number of created electrons/ions is proportional to the initial ionization. Instead of reading out the signal from the wires it is read out from the strips of the cathode plane, see Fig.~\ref{fig:mwpcs_operation}. The signal distribution over the strips is analyzed to provide a high position resolution.\newline
\begin{figure}[htpb]
    \centering
    \includegraphics[width=\textwidth,height=9cm,keepaspectratio=true]{Figures/mwpcs_operation.png}
    \caption{
    Schematic representation of the charge distribution when reading out the signal from the cathode pads (red) while the avalanche is created at the anode wire (blue) close to the symbol indicating the incident ion. For details see Table~\ref{table:mwpcs_tecs}.
    } 
    \label{fig:mwpcs_operation}
\end{figure}

In the S444 experiment four MWPCs were installed:\newline
\begin{enumerate}
...
\item MWPC1: $88$ cm downstream to the target for tracking in $x$ and $y$ of the outgoing ions.
\item MWPC2: $154$ cm downstream also for tracking in $x$ and $y$ of the fragment to determine the scattering angle.
\item MWPC3: after the GLAD magnet. The $x$ position of this detector gives the information about the magnetic deflection of the outgoing fragments.  %todo: find out position relative to Sofia TOFW
\end{enumerate}
...
\fontsize{10pt}{10pt}\selectfont
	...
	Wire diameter & 50 $\mu$m\\
        ...
        %%hline
		Rate capability & $\lesssim$ $100$ kHz \\
        \hline
	%\hspace
	\vspace{2\baselineskip}\\
        \multicolumn{2}{c}{\textbf{MWPC0}} \\ 
        \hline
	X pads & 64 pads, up/down readout \\
	...
	%\hspace
        ...
        \hline
	X pads & 64 pads, up/down readout\\
	Y pads & 40 pads, horizontally segmented (5 mm width)\\
	Active surface & 200 $\times$ 200 mm$^2$ \\
	$\Delta$X & 200 $\mu$m\\
	$\Delta$Y & 1 mmm\\
	\hline
...
\end{table}
\subsubsection{Multi Sampling Ionization Chambers}\label{sec:ionisation_chambers}
For the S444 experiment at R3B two types of \textbf{M}ulti \textbf{S}ampling \textbf{I}onisation \textbf{C}hambers (MUSICs) were installed: the R3B MUSIC, centered 153 cm upstream to the target, and the TWIN MUSIC, 132 cm downstream to the target. Ionization chambers are gas-filled detectors used to track charged particles and measure their specific energy loss with high precision. Both multi sampling ionization chambers consist of a cathode plane, a Frisch grid  and an anode plane, with a series of anode strips. When a charged particle crosses the chamber the gas gets ionized along the trajectory and the created electrons and ions are separated by the electric field. While the ions drift towards the cathode plane the electrons move to the anodes where each anode is read out separately. Since the energy loss of the passing through fully stripped ions is in first order proportional to the square of its nuclear charge ($\Delta E \sim \frac{Z^2}{\beta^2}$) the signal from the anodes allow to determine the charge of the particle especially if the velocity is measured separately by e.g. ToF. Moreover multi-sampling ionization chambers measure the drift time of the electrons created by the ionization process  on each anode. Assuming a constant electron drift velocity ($ v_{\text{drift}} \sim 40\,\text{mm}/\mu\text{s}$) over the gaseous volume the time information of each anode signal can be used to reconstruct the position along the electric field of the passing through particle.\newline
\textbf{R3B MUSIC}\newline
The R3B MUSIC, installed 153 cm upstream to the target (see Fig.~\ref{fig:setup_target_region}), is used to measure both the  charge of of the incoming ion before impinging on the target and the horizontal component of its polar angle. The detector has an active gaseous dimension of 20 x 20 x 40 $cm^3$, confined on one side by a cathode plane and on the other side by an anode plane segmented in 10 anodes (8 readout anodes and 2 screen anodes). For the technical specifications, see table \ref{table:r3bmusic_tecs}. \newline
\begin{table}[h!]
    \fontsize{10pt}{10pt}\selectfont
    \centering
    \begin{tabular}{ll}
    %\begin{tabular}{cc}
        \hline
        \textbf{Dimensions} & \\ 
	Detector dimension: & $51\times54\times53\, \text{cm}^3$ \\ 
	Active dimension: & $20\times20\times40\, \text{cm}^3$\\
	Dimension of one anode:& $20\times20\times5\, \text{cm}^3$\\
	Dimension of one screen anode:& $20\times20\times2\, \text{cm}^3$\\
	Installed Frisch grid next to anode plane\\
	\textbf{Gas} &\\
	P75 (Ar 25\%, CH4 75\%) \\
	\textbf{Voltage} & \\
	Cathode (left to beam direction): &  $-(2-6)$kV \\
	Anode (right to beam direction): & $+300$V \\ [10pt]
	Rate capability & $\lesssim$ $50$ kHz \\
	\hline
    \end{tabular}
...
\newline
\textbf{TWIN MUSIC}\newline
The TWIN MUSIC is a double ionization chamber with one central cathode plane and two independent horizontal drift volumes and anode planes on each side. Each anode plane comprises 16 readout anodes and 2 additional screen anodes. Each readout anode is further segmented into an upper and a lower half, resulting in four distinct detector sections each of size 11 x 11 x 40 cm$^3$. For unambiguous orientation, “left” and “right” refer to the view in beam direction. The sections are defined as follows: section 1 -- right, lower; section 2 -- right, upper; section 3 -- left, lower; section 4 -- left, upper. As the TWIN MUSIC is placed 132~cm downstream to the target (see Fig.~\ref{fig:setup_target_region}) it is employed to measure charge and the horizontal component of the polar angle of the outgoing medium-to-heavy fragments. The detector was designed for the requirements in fission experiments where mostly two heavy-fission fragments are created. If each fragment is flying through one of the four sections (which is mostly the case due to momentum conservation rules) charge and angle of each fragment can be measured independently.\newline
To fulfill the required permanence of the field in both extended gaseous volumes (of dimension $11\times22\times40\,\text{cm}^3$) a Frisch grid is located 3~mm from the anode planes. The Frisch grid is metal mesh grid that shields the anode from the movement of ions produced during ionization process in the chamber ensuring that only the electrons that reach the anode contribute to the signal. Additionally, the shielding of the anodes by the Frisch grid account for the fast rise time of the signal at the anodes which diminishes pile-up effects and makes the detector high beam-rate capable (up to 50kHz for light ions). Further technical specifications are listed in Table~\ref{table:twin_tecs}.\\
The position resolution in TWIN MUSIC depends strongly on the signal-to-noise ratio, and therefore on the atomic number $Z$. For heavy ions a resolution on the order of $60\,\mu$m can be achieved, for light ions such as carbon only about $300\,\mu$m is reachable. Consequently, for the S444 experiment the upstream position was reconstructed using the MWPCs (MWPC1 and MWPC2), which provide a resolution of approximately $200\,\mu$m.
\newline
\begin{table}[h!]
    \fontsize{10pt}{10pt}\selectfont
    \centering
    ...
    %\begin{tabular}{cc}
        \hline
        \textbf{Dimensions} & \\ 
	Detector dimension: & $43\times48\times55\, \text{cm}^3$ \\ 
	Active dimension: & two halves, each $11\times22\times40\,\text{cm}^3$\\
	Distance central cathode - Frisch grid: & 11~cm\\
	Distance Frisch grid from anode planes: & 3~mm\\ %todo 11cm +3 mm = 11cm?!?
	\textbf{Gas} &\\
	...
	Anode planes: & $+600$ V \\
	Frisch Grid: & $+250$ V\\
	\textbf{Resolutions} (for heavy ions) & \\
	$\Delta E / E$ & $< 5$\% FWHM, total < 2\% FWHM \\
	$\Delta X$ & $< 60\, \mu$m\\
	$\Delta \theta$ & $\approx$ 0.3 mrad\\[10pt] 
	Rate capability & $\lesssim$ $50$ kHz \\
	\hline
    \end{tabular}
...
The SOFIA Start detector is positioned right after the R3B MUSIC ionization chamber (see Fig.~\ref{fig:setup_target_region}) and gives a reference time for the incoming ion. It is a 1 mm thin scintillating plastic blade read out by two PMTs attached to the short sides (left and right) of the slab. The fast signal of both PMTs is fed into constant fraction discriminators (CFD) and input for time to digital converters (VFTX modules~\footnote{Technical details for the VFTX modules are available at \url{https://www.gsi.de/fileadmin/EE/Module/Dokumente/VFTX_Manual_2_0.pdf}.}):
\[t_{start} = 0.5 \cdot (t_{left}+t_{right}) \]\newline
To shield the plastic detector from daylight it is wrapped in Mylar foil (150$\mu$m thickness).
\subsubsection{GLAD Magnet}
R3B is a classical Dipole Spectrometer with its GSI Large Acceptance zero degree superconducting Dipole magnet (GLAD) in the center of the R3B setup in the Cave C (see Fig.~\ref{fig:setup_target_region}). Its adjustable field integral up to 5 Tm and a high acceptance range are crucial for the identification of highly asymmetric reaction fragments. The homogeneous magnetic field in GLAD allows to achieve momentum resolutions $\Delta p/p$ of 10$^{-3}$ in combination with the dedicated tracking system.\newline
...
The default bending angle of the beam with respect to the beam line was set to 18$^{\circ}$. Herefore the coil currents where adjusted according to the different beam energies:
...
\subsubsection{The CALIFA Calorimeter}\label{sec:califa}
The \textbf{CAL}orimeter for \textbf{I}n \textbf{F}light detection of $\gamma$-rays and high energy charged p\textbf{A}rticles, CALIFA, is one of the main detector components of the R3B setup. It surrounds the target area and covers the full azimuthal range and a polar angular acceptance from 7$^{\circ}$ up to 140$^{\circ}$ in the target region in its final configuration. The calorimeter serves for the detection of gamma rays in the energy region 100 keV $\lesssim$ E$_{\gamma}$ $\lesssim$ 30 MeV and light charged particles, mostly protons, up to several hundred MeV. To fulfill the demands requested by the different experimental campaigns an energy resolution of $\frac{\Delta E}{E}$ $\sim$ 6\%/$\sqrt{E[\text{MeV}]}$ (FWHM) in the gamma-ray energy regime is achieved. For proton energies in the range 100 $\leq \text{E}_p \leq$ 300 AMeV, the corresponding energy resolution is approximately $\frac{\Delta E}{E}\sim 1\% \cdot\frac{\sqrt{100\,\text{MeV}}}{\sqrt{E[\text{MeV}]}}$.\\
%The development, implementation, and operation of the full readout electronics for CALIFA are carried out entirely by the TUM group.\\[1em]
The full readout electronics of CALIFA -- including design, development, implementation, and operation -- are provided entirely by the TUM group, which has led all associated hardware and firmware developments~\cite{bendel2014entwicklung, Bendel:98055, winkel2011implementierung, winkel2016komplexe, pklenze}. \\[1em]
\textbf{Geometry}\newline
The CALIFA detector is designed as a highly segmented detector with 2544 CsI(Tl) crystals. Since experiments in the R3B setup operate in relativistic kinematics both the incoming ions as well as the the measured particles originating from reactions experience relativistic effects, more precisely the relativistic Doppler effect. The emitted gamma rays and protons are not isotropically distributed around the source region but are instead boosted in the forward direction. Furthermore, the energy measured in the laboratory frame differs from the kinetic energy in the rest frame of the incoming ion due to relativistic effects.\newline
The relativistic Doppler effect has a huge impact on the geometric design and requirements of CALIFA. Herefore the detector was subdivided into two polar angle ranges, see Fig.~\ref{fig:califa_sec}:
\begin{figure}[htpb]
    \centering
    %\includegraphics[width=\textwidth,keepaspectratio=true]{Figures/califa_section_overview.png}
    %\includegraphics[width=\textwidth,keepaspectratio=true]{Figures/califa_section_overview_hq.png}
    \includegraphics[width=\textwidth,keepaspectratio=true]{Figures/califa_section_overview_hq_labels.png}
    \caption{
    Schematic view of the Barrel and Endcap(iPhos and CEPA) segments of CALIFA and the according angular and energy distribution of emitted $\gamma$ rays (isotropic and mono-energetic in the projectile frame at beam energy of 700 AMeV). Figure adapted from Ref.~\cite{tdr:barrel}.
    }
...
\end{equation}
where $N_{f}$ is the amplitude of the fast component and $N_{s}$ the amplitude of the slow component. Accordingly $\tau_{f}$ the life time of the fast component ($\tau_{f} \approx 650-770 ns$ ) and $\tau_{s}$ the lifetime of the slow component ($\tau_{s} \approx 3.2 - 3.5\mu s$). It has been found that the proportion between the two components is energy and particle dependent. This property can be used to identify isotopes by extracting the $N_{f}$ and $N_{s}$ values from pulse shape analysis (PSA) on the according light emission response\footnote{The method has been implemented in the CALIFA Firmware as \textit{Quick Particle Identification} -- QPID. For more information see~\cite{winkel2011implementierung} and~\cite{winkel2016komplexe}.}.\\[1em]
%, as shown in Fig.~\ref{fig:qpid_califa}.\\[1em]  
%\begin{figure}[ht!]
%    \centering
%    %\includegraphics[width=\textwidth,keepaspectratio=true]{Figures/qpid_overview.pdf}
%    \includegraphics[width=0.85\textwidth,keepaspectratio=true]{Figures/qpid_overview_cut.png}
%    \caption{
%    Quick Particle Identification(QPID) via fast and slow component $N_{f} / N_{s}$. Each correlation band corresponds to a nuclide, from Ref.~\cite{winkel2016komplexe}.
%    }
%    \label{fig:qpid_califa}
%\end{figure}
\textbf{From scintillation light to electrical signal}\newline
To extract all features of the signal response for later analysis in spectroscopic quality a fully digital DAQ based on the FEBEX 3B module developed at GSI is implemented~\footnote{A detailed description can be found in Philipp Klenze's~\cite{pklenze} and Max Winkel's thesis~\cite{winkel2016komplexe}.}. 

%The scintillation light produced at different points inside the crystal has first to be transported to the back-end of the crystal. The optimum design has been determined by wrapping the be frustrum-shaped crystals into enhanced specular reflector (ESR) foil which provides excellent reflectivity.\footnote{Detailed information about the crystal wrapping and LAAPD gluing can be found in this work:\cite{hartigevolution}}. Finally, a large-area avalanche photodiode (LAAPD), specifically the Hamamatsu S12102 model~\cite{hamamatsuS8664}, is attached at the rear end of each crystal to enable the detection of scintillation light. Avalanche photodiodes (APDs) operate on the same fundamental principle as conventional photodiodes, converting scintillation light into electrical signals. Both offer the advantage of being insensitive to magnetic fields, making them particularly suitable for applications close to the GLAD magnet. As a result of an additional highly doted p-layer a region with very high field is formed which accounts for amplification factors up to $\approx$ 100.\newline
...
\begin{figure}[h!]
	...
	\begin{tabular}{cc}\hline 
  	...
  	Plastic dimensions & 5 $\times$ 32 $\times$ 600 mm$^3$ \\
  	Detector dimension & 5 $\times$ 900 $\times$ 600 mm$^3$ (28 plastics) \\ 
...
For the detection of neutrons emitted from the forward going fragments  the New Large-Area Neutron Detector (NeuLAND) is installed at zero degrees after GLAD. In its final design it will consist of 30 double planes with each 100 plastic scintillators of size $5\times5\times250~\text{cm}^3$ providing an active detector surface of $2.5\times2.5~\text{m}^2$ and thickness of 3~m. Its high detection efficiency, a time resolution of $\sigma_t \le 150\,ps$ and high multi-neutron efficiency are crucial detector features for complete kinematics experiments at R3B. A comprehensive analysis of the detector's resolution and efficiency is provided in Refs.~\cite{boretzky2021neuland, boretzky2014neuland}.\newline
