...
\quad \textbf{Neutron skin thickness $\Delta_{np}$}: quantifies the difference between the root-mean-square (rms) radii of the neutron ($R_n$) and proton ($R_p$) distributions in a nucleus, i.e., $\Delta_{np} = R_n - R_p$. This quantity provides crucial insights into the Equation of State (EoS) of asymmetric nuclear matter, particularly the density dependence of the symmetry energy.

It has been shown by the seminal work of X. Roca-Maza et al.~\cite{roca2011neutron} that the neutron skin thickness $\Delta_{np}$ linearly correlates with the slope parameter L of the nuclear symmetry energy, as illustrated in Fig.~\ref{fig:skin_thickness_vs_L}. This strong correlation implies that a precise measurement of $\Delta_{np}$ can significantly constrain the value of L. Current experimental constraints on L are primarily derived from experiments such as the PREX (Lead Radius Experiment), which utilized parity-violating electron scattering to measure the weak charge distribution of $^{208}$Pb. The PREX experiment measured the parity-violating asymmetry $A_{PV}$ in polarized electron scattering on $^{208}$Pb with an accuracy of 3\%, resulting in a constraint on $L$ of $\pm 40$ MeV~\cite{roca2011neutron}.

\begin{figure}[htpb]
    \centering
    \includegraphics[width=\textwidth,height=10cm,keepaspectratio=true]{Figures/neutron_skin_vs_L.png}
    \caption{
        Correlation between $\Delta_{np}$ and parameter L, from analysis in~\cite{roca2011neutron} for the neutron skin of $^{208}$Pb. Relativistic and non-relativistic models with different relativistic grounds are displayed. Figure taken from~\cite{roca2011neutron}.
    }
...
\end{figure}
The R3B Setup which allows high precision particle spectroscopy is well suited for such kind of experimental approach. In 2018 the pilot experiment S473 was carried out with stable $^{120}$Sn beam an carbon target and in 2021 the follow up experiment S515 with $^{124,128,132,134}$Sn beam. 
First experiments 
