...
\quad \textbf{Neutron skin thickness $\Delta_{np}$}: quantifies the difference between the root-mean-square (rms) radii of the neutron and proton distributions in a nucleus. It has been shown by the pivotal work of X. Roca-Maza et. al.~\cite{roca2011neutron} that the neutron skin thickness $\Delta_{np}$ linearly correlates with the slope parameter L as shown in Fig.~\ref{fig:skin_thickness_vs_L}. A precise $\Delta_{np}$ measurement could drastically constrain the parameter L. The data point in Fig.~\ref{fig:skin_thickness_vs_L} is from the PREX experiment where the slope parameter L was measured via the parity-violating asymmetry $A_{PV}$ in polarized electron scattering on $^{208}$Pb with an accuracy of 3\% and a constrain on L to $\pm 40$MeV.\\
\begin{figure}[htpb]
    \centering
...
With the decisive work of T. Aumann, C.A. Bertulani et al.~\cite{aumann2017peeling} it has been shown that the neutron skin thickness is accessible  and sensitive via the neutron-removal cross section for heavy neutron rich nuclei. This proposal involves the measurement of the neutron removal cross section $\sigma_{\Delta N}$ along the isotopic chain of thin $^{124-134}$Sn. The values for the neutron removal cross section can then be compared to various predictions from theoretical density distributions from relativistic mean-field calculations directly correlated to the resulting value for the slope parameter L as shown in Fig.~\ref{fig:sub_nrem_vs_a} and~\ref{fig:sub_thickness_vs_l}. In their work they demonstrated that the slope parameter L of the symmetry energy could be constrained down to $\pm 10$ MeV by such a measurement, with a 2\% accuracy of the measured and calculated cross sections.
\begin{figure}
	\centering
