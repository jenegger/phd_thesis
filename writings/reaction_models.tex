...
\begin{itemize}
    ...
    \item \textbf{Multiple Scattering Formalism:} The total scattering amplitude is constructed by summing over all possible interactions between the projectile and individual nucleons within the target. This requires integration over the spatial distributions of all participating nucleons, which becomes computationally intensive for large systems. To address this, additional simplifications are introduced, leading to the so-called \textit{optical-limit approximation}, discussed in Subsection~\ref{sec:eol}.
    
    \item \textbf{Impact Parameter Formalism:} The cross-section is expressed as an integral over the transverse distance (impact parameter) between the centers of the projectile and the target, see Fig.~\ref{fig:impact_par_glauber}. This semiclassical treatment enables a geometric interpretation of the interaction and supports a probabilistic description of scattering.
\end{itemize}
 \begin{figure}[htbp]
     \centering
...
For the introduction to this discussion is the elastic scattering process where neither the projectile nor the target gets excited or nucleons get knocked out. A good description of the elasitc scattering process allows us to later derive the reaction and the total cross section.\\
...
The central goal is to find the wavefunction $\psi_{\vec{k}}(\vec{r})$ that fulfills equation~\ref{eq:schroedinger}  which consists of the sum of the incoming plane wave $\varphi_{\vec{k}}(\vec{r})$ and the outgoing spherical wave $\psi_{\vec{k}}(\vec{r})$, i.e.:
\begin{equation}
