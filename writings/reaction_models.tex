...
The central goal is to find the wavefunction $\psi_{\vec{k}}(\vec{r})$ that fulfills equation~\ref{eq:schroedinger}  which consists of the sum of the incoming plane wave and the outgoing spherical wave, i.e.:
\begin{equation}
\psi_{\vec{k}}(\vec{r}) = A [e^{ikr} + f(\theta) \frac{e^{ikr}}{r}]
\end{equation}
